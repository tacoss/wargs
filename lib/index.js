...
module.exports = (argv, opts, cb) => {
  ...
  function c(val) {
    return opts.camelCase
      ? val.replace(reCamelCase, ($0, $1) => $1.toUpperCase()) : val;
  }
  ...
  function a(prop, old) {
    ...
    if (prop.type === 'flag') {
      /* istanbul ignore else */
      if (opts.arrays.indexOf(prop.key) > -1) {
        /* istanbul ignore else */
        if ((old && old.key !== prop.key) || !prop.value || old === null) {
          // skip non-consecutive flags
          return;
        }
      }

      n(prop.key, d(prop, prop.key.indexOf('no-') !== 0));
    }
  ...
  function s(key) {
    ...
    $.key = opts.aliases[$.key] || $.key;
    $.boolean = opts.booleans.indexOf($.key) > -1;
  ...
  function l(key) {
    ...
    if (m) {
      ...
      /* istanbul ignore else */
      if (m[0].charAt() === '-') {
        const $ = s(m[0].substr(0, m[0].length - 1));

        /* istanbul ignore else */
        if (opts.booleans.indexOf($.key) > -1) {
          throw new Error(`Unexpected value for '-${$.key}' flag`);
        }
