...
const reValueWithSpaces = /(?:^| )([-~+.\w]+)([\s:=]?(["'])(?:(?!\3)[\s\S]|\\\3)*\3)(?: |$)/;
...
module.exports = (value, opts, cb) => {
  ...
  function q(key, value, quote) {
    ...
    /* istanbul ignore else */
    if (key.charAt() === '-') {
      flags[c(key.replace(reTrimDashes, ''))] = f(value);
      return;
    }
    ...
    /* istanbul ignore else */
    if (char === ' ') {
      data[c(key)] = opts.asBool;
      data[c(value)] = opts.asBool;
      return;
    }

    data[c(key + value)] = opts.asBool;
  }
