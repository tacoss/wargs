...
module.exports = (argv, opts, cb) => {
  ...
  // trim leading dashes
  function t(key) {
    return key.replace(reTrimDashes, '');
  }

  // append normalized flags
  function n(key, value) {
    if (key.substr(0, 3) === 'no-') {
      flags[key.substr(3)] = !value;
    } else {
      flags[key] = value;
    }
  }
  ...
  if (!Array.isArray(argv)) {
    ...
    do {
      test = argv.match(reValueWithSpaces);

      /* istanbul ignore else */
      if (test) {
        argv = argv.replace(test[0], ' - ');
  ...
  argv.concat('-').reduce((prev, cur) => {
    ...
    /* istanbul ignore else */
    if (key !== '-' && key.charAt() === '-') {
      if (key.substr(0, 2) === '--' || opts.booleans.indexOf(key.substr(-1)) === -1) {
        if (opts.booleans.indexOf(t(key)) > -1 && reMatchKeyValue.test(cur)) {
          a(cur);
  ...
  return { _, cmd: __, data, flags, params};
};
